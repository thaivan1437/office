...
function showidlist1($id){
	global $d;
	
	$d->reset();
	$sql="select id, tenkhongdau from #_product_list where tenkhongdau='".$id."' order by id desc limit 0,1";
	$d->query($sql);
	$rs=$d->result_array();
	if(!empty($rs))
	$kq=$rs[0]["tenkhongdau"];
...
function showidlist($id){
	...
	$d->reset();
	$sql="select id, tenkhongdau from #_product_list where find_in_set(".$id.", dist) order by id desc";
	$d->query($sql);
	$rs=$d->result_array();
	
	$kq=$rs[0]["tenkhongdau"]."-".$rs[0]["id"];
	return $kq;
}
function showPrice($gia){
	global $row_setting;
	if($row_setting["donvi"]==1){
		$kq=$gia.' USD';
	}else{
		$kq=number_format(($gia*$row_setting["tigia"]),"0",",",".").' VNĐ';
	}
    //dump($kq);
	return $kq;
}
...
function getTenkhongdauList($id){
	global $d;
	$d->reset();
	$sql="select tenkhongdau from #_place_dist where id='".$id."'";
	$d->query($sql);
	$rs=$d->fetch_array();
	if($d->num_rows()>0){
		return showidlist($id).'-'.$rs["tenkhongdau"];
	}else{
		return "van-phong";
	}
...
function show_ward($province){
	global $d;
	$d->reset();
	$sql="select * from #_place_ward where id='".$province."'";
	$d->query($sql);
	$rs_dis=$d->fetch_array();
	
	return $rs_dis['ten'];
}
function show_street($province){
	global $d;
	$d->reset();
	$sql="select * from #_place_street where id='".$province."'";
	$d->query($sql);
	$rs_dis=$d->fetch_array();
