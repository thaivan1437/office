...
function get_items(){
	...
	$url="index.php?com=couple&act=man";
	$maxR=20;
	$maxP=10;
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=couple&act=man");
	...
	if($id){
		...
		if($d->update($data)){
			if(isset($_POST['referer_link']))
				...
				redirect("index.php?com=couple&act=man&curPage=".$_REQUEST['curPage']."");
		}
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=couple&act=man");
	}else{		
		...
		if($d->insert($data))
		{
			...
			if (!$mail->Send()) {
				transfer("Có lỗi xảy ra!", "index.php?com=couple&act=man");
			} else {
				$data2["active"]=1;
				...
				transfer("Gửi coupon cho khách hàng và doanh nghiệp thành công!<br/>", "index.php?com=order&act=man");
			}
			
			redirect("index.php?com=couple&act=man");
		}
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=couple&act=man");
	}
}
