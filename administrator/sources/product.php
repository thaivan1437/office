...
$urldanhmuc.= (isset($_REQUEST['dist'])) ? "&dist=" . addslashes($_REQUEST['dist']) : "";
...
function saveImageWaterMark($img,$img_condau){
    ...
    $image_type = end(explode(".",$img)); //file type
    ...
    switch(strtolower($image_type)){ //determine uploaded image type 
		...
		case 'image/png': 
			...
			break;
		case 'image/gif':
			$image =  imagecreatefromgif($image_temp);
			break;          
	...
	$w0 = $info[0];//chi?u dài hình g?c
	$w1 = $info1[0];// chi?u dài watermark (hình con d?u)
	$h0=$info[1];  // chi?u cao hình g?c
	$h1 = $info1[1]; // chi?u cao watermark (hình con d?u)
	//$watermark_pos_x =0;//($w0-$w1)/2;// canh gi?a trái ph?i // 
...
function get_items() {
	...
	if ((int) $_REQUEST['dist'] != '') {
        $sql.=" and  dist='" . $_REQUEST['dist'] . "'";
    }
...
function save_item() {
	...
	$d->reset();
	$sql="select photo from #_setting limit 0,1";
	$d->query($sql);
	...
	$data['tentoanha'] = $_POST['tentoanha'];
    $data['vitri'] = $_POST['vitri'];
    $data['template_vp'] = $_POST['template_vp'];
    $data['gia_vp_ao'] = $_POST['gia_vp_ao'];
    $data['gia_cho_ngoi'] = $_POST['gia_cho_ngoi'];
    $data['gia_thue_phong'] = $_POST['gia_thue_phong'];
    $data['chu_dau_tu'] = $_POST['chu_dau_tu'];
    $data['loai_vp'] = $_POST['loai_vp'];
    $data['gia_theo_thang'] = $_POST['gia_theo_thang'];
    $data['vat'] = $_POST['vat'];
    $data['tangchothue'] = $_POST['tangchothue'];
	
	$data['type'] = $type;
	$data['loai'] = (int)$_POST['loai'];
	$data['city'] = (int)$_POST['province'];
	$data['dist'] = (int)$_POST['district'];
	$data['ward'] = (int)$_POST['ward'];
	$data['id_giasearch'] = (int)$_POST['id_giasearch'];
	$data['id_dientich'] = implode($_POST['id_dientich'],",");
	$data['street'] = (int)$_POST['street'];
    ...
    $data['baogomdienlanh'] = isset($_POST['baogomdienlanh']) ? 1 : 0;
	$data['h1'] = $_POST['h1'];
	$data['h2'] = $_POST['h2'];
	$data['h3'] = $_POST['h3'];
	...
	$data['ketcau'] = magic_quote($_POST['ketcau']);
	$data['dientichxaydung'] = magic_quote($_POST['dientichxaydung']);
	$data['dientichsudung'] = magic_quote($_POST['dientichsudung']);
	
	
	//trọn gói
	//$data['dientichtrongoi'] = $_POST['dientichtrongoi'];
	$data['sochongoi'] = $_POST['sochongoi'];
	$data['giolamviec'] = magic_quote($_POST['giolamviec']);
	$data['loaichothue'] = $_POST['loaichothue'];
	$data['giatrongoi'] = $_POST['giatrongoi'];

	
	
	//tòa nhà
	$data['tinhtrang'] = $_POST['tinhtrang'];
	$data['thoihan'] = $_POST['thoihan'];
	$data['giatoanha'] = $_POST['giatoanha'];
	
	$data['ketcautoanha'] = magic_quote($_POST['ketcautoanha']);
	$data['phidichvu'] = magic_quote($_POST['phidichvu']);
	$data['uudiem'] = magic_quote($_POST['uudiem']);
	$data['phongthuy'] = magic_quote($_POST['phongthuy']);
	$data['bando'] = magic_quote($_POST['bando']);
	
	$data['phiql'] = $_POST['phiql'];
	$data['phixemay'] = $_POST['phixemay'];
	$data['phioto'] = $_POST['phioto'];
	$data['tiendien'] = $_POST['tiendien'];
	$data['tiendienlanh'] = $_POST['tiendienlanh'];
	$data['tiendatcoc'] = $_POST['tiendatcoc'];
	$data['thanhtoan'] = $_POST['thanhtoan'];
    ...
    if ($id) { 
        ...
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product, $file_name)) {
            $data['photo'] = $photo;
			//saveImageWaterMark(_upload_product.$data['photo'],_upload_hinhanh.$condau["photo"]);
        ...
        if ($d->update($data)){
			$d->reset();
			$sql = "delete from #_product_dientich where id_product='" . $id . "'";
			$d->query($sql);
			$array_dt = explode("-",$_POST['dientich']);
			foreach($array_dt as $v){
				$data2["dientich"]=$v;
				$data2["id_product"]=$id;
				$d->reset();
				$d->setTable('product_dientich');
				$d->insert($data2);
			}
	            ...
	            for ($i = 0; $i < $fileCount; $i++) { 
					...
					if(empty($file_info)) // No Image?
					{
						...
						if(move_uploaded_file($myFile["tmp_name"][$i], _upload_product."/".$file_name."_".$myFile["name"][$i])){		            							
							...
							//saveImageWaterMark(_upload_product.$data1['photo'],_upload_hinhanh.$condau["photo"]);
							$data1['id_photo'] = $id;
							$data1['hienthi'] = 1;
		...
		if ($photo = upload_image("file1", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product, $file_name1)) {
            $data['photo1'] = $photo;
			//saveImageWaterMark(_upload_product.$data['photo1'],_upload_hinhanh.$condau["photo"]);
            //$data['thumb'] = create_thumb($data['photo'], 290, 350, _upload_product, $file_name, 2);
        }
		if ($file = upload_image("file2", 'pdf|doc|docx|xls|xlsx|rar|zip|PDF|DOC|DOCX|XLS|XLSX|RAR|ZIP', _upload_product, $file_name2)) {
            $data['file'] = $file;
        ...
        if ($d->insert($data)) {
			...
			$d->reset();
			$sql = "delete from #_product_dientich where id_product='" . $id_insert . "'";
			$d->query($sql);
			$array_dt = explode("-",$_POST['dientich']);
			foreach($array_dt as $v){
				$data2["dientich"]=$v;
				$data2["id_product"]=$id_insert;
				$d->reset();
				$d->setTable('product_dientich');
				$d->insert($data2);
			}
...
function get_lists() {
    ...
    if(!empty($_POST)){
		...
		if($multi=='show'){
			...
			redirect("index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat);			
		}
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat." ";
    $maxR = 15;
    $maxP = 10;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_list() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."");
    $sql = "select * from #_product_list where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."");
    $item = $d->fetch_array();
}
...
function save_list() {
	...
	$data['com'] = $subcat;
	$data['type'] = $type;
	$data['value_menu'] = $_POST['value_menu'];
	$data['h1'] = $_POST['h1'];
	$data['h2'] = $_POST['h2'];
	$data['h3'] = $_POST['h3'];
	$data['fontawesome'] = $_POST['fontawesome'];
	$data['typeloai'] = $_POST['typeloai'];
	$data['dist'] = implode(",",$_POST['dist']);
	if ($id) {
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product, $file_name)) {
            $data['photo'] = $photo;
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."&curPage=" . $_REQUEST['curPage'] . "");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."");
    }else {
        ...
        if ($d->insert($data))
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."");
    }
}
...
function delete_list() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list&type=".$type."&subcat=".$subcat."");
}
...
function get_photos() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat;
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_photo() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_photo() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
        redirect("index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
    } { // them moi
        for ($i = 0; $i < 5; $i++) {
            if ($photo = upload_image("file" . $i, 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_product, $file_name . $i)) {
                ...
                $d->setTable('product_hinhanh');
                if (!$d->insert($data))
                    transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
            }
        }
        redirect("index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
    }
...
function delete_photo() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_photo&idc=" . $_REQUEST['idc'] . "&subcat=".$subcat);
}
function get_tabs() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "";
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_tab() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");

    $d->setTable('product_tab');
    $d->setWhere('id', $id);
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_tab() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
        redirect("index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
    } { // them moi
        $data['stt'] = $_POST['stt'];
		$data['ten_vi'] = $_POST['ten_vi'];
		$data['ten_en'] = $_POST['ten_en'];
		...
		$d->setTable('product_tab');
		if (!$d->insert($data))
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
		else
        redirect("index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
    }
}
...
function delete_tab() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_tab&idc=" . $_REQUEST['idc'] . "");
}
...
function get_colors() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "";
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_color() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");

    $d->settable('product_color');
    $d->setWhere('id', $id);
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_color() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
        redirect("index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
    } { // them moi
        $data['stt'] = $_POST['stt'];
		$data['ten'] = $_POST['ten'];
		$data['color'] = $_POST['color'];
		...
		$d->settable('product_color');
		if (!$d->insert($data))
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
		else
        redirect("index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
    }
}
...
function delete_color() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_color&idc=" . $_REQUEST['idc'] . "");
}
...
function get_sizes() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "";
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_size() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");

    $d->settable('product_size');
    $d->setWhere('id', $id);
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_size() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
        redirect("index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
    } { // them moi
        $data['stt'] = $_POST['stt'];
		$data['ten'] = $_POST['ten'];
		$data['gia'] = (int)$_POST['gia'];
		...
		$d->settable('product_size');
		if (!$d->insert($data))
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
		else
        redirect("index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
    }
}
...
function delete_size() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_size&idc=" . $_REQUEST['idc'] . "");
}
...
function delete_binhluan() {
    ...
    if (isset($_GET['id'])) {
        ...
        transfer("Không nhận được dữ liệu", "index.php?com=product&act=duyetbl");
}
