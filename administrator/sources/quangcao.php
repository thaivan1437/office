...
function get_photo() {
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=quangcao&act=man_photo");
    $item = $d->fetch_array();
}
...
function save_photo() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=quangcao&act=man_photo");
        ...
        for ($i = 0; $i < 5; $i++) {
            if ($data['photo'] = upload_image("file" . $i, 'jpg|jpg|png|gif|JPG|jpeg|JPEG', _upload_hinhanh, $file_name . $i)) {
                ...
                $d->setTable('quangcao');
                if (!$d->insert($data))
                    transfer("Lưu dữ liệu bị lỗi", "index.php?com=quangcao&act=man_photo");
            }
        }
