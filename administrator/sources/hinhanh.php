...
function get_items() {
    ...
    if (@$_REQUEST['update'] != '') {
        ...
        //echo "id:". $spdc1;
        if ($spdc1 == 0) {
            $sqlUPDATE_ORDER = "UPDATE table_hinhanh SET noibat ='" . $noibat . "' WHERE  id = " . $id_up . "";
...
function get_item() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=hinhanh&act=man");
    $item = $d->fetch_array();
...
function save_item() {
    ...
    if ($id) {
        ...
        if ($d->update($data)){
			...
			redirect("index.php?com=hinhanh&act=man");
		}
            
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=hinhanh&act=man");
    }else {
        ...
        if ($d->insert($data)){
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=hinhanh&act=man");
    }
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=hinhanh&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=hinhanh&act=man");
}
...
function get_cat() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=hinhanh&act=man_cat");
    $item = $d->fetch_array();
}
...
function get_photo() {
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=hinhanh&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_photo() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=hinhanh&act=man_photo&idc=" . $_REQUEST['idc'] . "");
        ...
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_hinhanh, $file_name)) {
            ...
            $d->setTable('hinhanh_hinhanh');
            if (!$d->insert($data))
                transfer("Lưu dữ liệu bị lỗi", "index.php?com=hinhanh&act=man_photo&idc=" . $_REQUEST['idc'] . "");
        }
    }
    redirect("index.php?com=hinhanh&act=man_photo&idc=" . $_REQUEST['idc'] . "");
}
