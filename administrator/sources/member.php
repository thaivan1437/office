...
function get_lists() {
    ...
    $sql = "select * from #_member where com='".$_REQUEST['type']."' ";
    ...
    $url = "index.php?com=member&act=man_list&type=".$_REQUEST['type']."";
    $maxR = 20;
    $maxP = 10;
...
function get_list() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
    $sql = "select * from #_member where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
    $item = $d->fetch_array();
}
...
function save_list() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=member&act=man_list&curPage=" . $_REQUEST['curPage'] . "&type=".$_REQUEST['type']."");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
    }else {
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_member, $file_name)) {
            $data['photo'] = $photo;
        }
        ...
        $d->setTable('member');
        if ($d->insert($data))
            redirect("index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
        else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
    }
}
...
function delete_list() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            redirect("index.php?com=member&act=man_list");
        else
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=member&act=man_list&type=".$_REQUEST['type']."");
}
...
function get_photos() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."";
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_photo() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_photo() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
        redirect("index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
    } { // them moi
        for ($i = 0; $i < 5; $i++) {
            if ($photo = upload_image("file" . $i, 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_member, $file_name . $i)) {
                ...
                $d->setTable('member_hinhanh');
                if (!$d->insert($data))
                    transfer("Lưu dữ liệu bị lỗi", "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
            }
        }
        redirect("index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
    }
...
function delete_photo() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=member&act=man_photo&idc=" . $_REQUEST['idc'] . "&type=".$_REQUEST['type']."");
}
...
function get_duyetbl() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=member&act=duyetbl";
    $maxR = 20;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function delete_binhluan() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            redirect("index.php?com=member&act=duyetbl");
        else
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=member&act=duyetbl");
        ...
        transfer("Không nhận được dữ liệu", "index.php?com=member&act=duyetbl");
}
