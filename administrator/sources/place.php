...
function get_dists(){
	...
	$sql= "SELECT count(id) AS numrows FROM #_place_dist where id_city=1";
	$d->query($sql);	
	$dem=$d->fetch_array();
	$totalRows=$dem['numrows'];
	$page=$_GET['p'];
	...
	$sql = "SELECT * from #_place_dist where id_city=1 order by id  limit $bg,$pageSize";		
	$d->query($sql);
	$items = $d->result_array();	
...
function get_wards(){
	...
	$sql= "SELECT count(id) AS numrows FROM #_place_ward  where id_city=1 ";
	$d->query($sql);	
	$dem=$d->fetch_array();
	$totalRows=$dem['numrows'];
	$page=$_GET['p'];
	...
	$sql = "SELECT * from #_place_ward  where id_city=1 order by id limit $bg,$pageSize";		
	$d->query($sql);
	$items = $d->result_array();	
...
function get_streets(){
	...
	$sql= "SELECT count(id) AS numrows FROM #_place_street where id_city=1 $where ";
	$d->query($sql);	
	$dem=$d->fetch_array();
	$totalRows=$dem['numrows'];
	$page=$_GET['p'];
	...
	$sql = "SELECT * from #_place_street where id_city=1 $where order by id limit $bg,$pageSize";		
	$d->query($sql);
	$items = $d->result_array();	
...
function save_street(){
	...
	if($id){
		$id =  themdau($_POST['id']);					
		$data['id_city'] = (int)$_POST['id_city'];	
		$data['id_dist'] = implode(",",$_POST['id_dist']);	
		$data['ten'] = $_POST['name'];	
		$data['tenkhongdau'] = changeTitle($_POST['name']);			
		$data['stt'] = $_POST['num'];			
		$data['hienthi'] = isset($_POST['active']) ? 1 : 0;		
