...
function get_items(){
	...
	$url="index.php?com=prices&act=man";
	$maxR=10;
	$maxP=4;
...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=prices&act=man");
	
	$sql = "select * from #_prices where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=prices&act=man");
	$item = $d->fetch_array();
}
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=prices&act=man");
	...
	if($id){ // cap nhat
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			header("Location:index.php?com=prices&act=man");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=prices&act=man");
	}else{ // them moi
		$data['ngaytao'] = time();
		$d->setTable('prices');
		if($d->insert($data)){
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=prices&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=prices&act=man");
	}else transfer("Không nhận được dữ liệu", "index.php?com=prices&act=man");
}
#--------------------------------------------------------------------------------------------- photo
