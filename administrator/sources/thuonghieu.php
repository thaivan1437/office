...
function get_photos() {
	...
	if(!empty($_POST)){
		...
		if($multi=='show'){
			...
			redirect("index.php?com=thuonghieu&act=man_photo&type=".$type);			
		}
    ...
    $url = "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo";
    $maxR = 10;
    $maxP = 4;
...
function get_photo() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_photo() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");


        header("Location:index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
    } { // them moi
        ...
        $d->setTable('thuonghieu');
        if ($d->insert($data)) {
            redirect("index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
        } else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
    }
}
...
function delete_photo() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=thuonghieu&type=".$type."&id_photo=".$id_photo."&act=man_photo");
}
?>
