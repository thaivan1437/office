...
function get_items() {
	...
	if(!empty($_POST)){
		...
		if($multi=='show'){
			...
			redirect("index.php?com=giasearch&act=man&type=".$type);			
		}
    ...
    $url = "index.php?com=giasearch&act=man";
    $maxR = 20;
    $maxP = 4;
...
function get_item() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=giasearch&act=man");

    $sql = "select * from #_giasearch where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=giasearch&act=man");
    $item = $d->fetch_array();
}
...
function save_item() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=giasearch&act=man");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=giasearch&act=man");
    }else {
        ...
        if ($d->insert($data))
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=giasearch&act=man");
    }
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=giasearch&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=giasearch&act=man");
}
