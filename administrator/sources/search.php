...
function get_items() {
    ...
    $url = "index.php?com=search&id_list=".$id_list."&act=man";
    $maxR = 20;
    $maxP = 4;
...
function get_item() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=search&id_list=".$id_list."&act=man");

    $sql = "select * from #_search where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=search&id_list=".$id_list."&act=man");
    $item = $d->fetch_array();
}
...
function save_item() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=search&id_list=".$id_list."&act=man");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man");
    }else {
        ...
        if ($d->insert($data))
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man");
    }
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=search&id_list=".$id_list."&act=man");
}
...
function get_cats() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=search&id_list=".$id_list."&act=man_cat";
    $maxR = 20;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_cat() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=search&id_list=".$id_list."&act=man_cat");

    $sql = "select * from #_search_item where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=search&id_list=".$id_list."&act=man_cat");
    $item = $d->fetch_array();
}
...
function save_cat() {
    ...
    if ($id) {
        ...
        if ($d->update($data)){
			...
			redirect("index.php?com=search&id_list=".$id_list."&act=man_cat&curPage=" . $_REQUEST['curPage'] . "");
		}
            
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man_cat");
    }else {
        $data['ten'] = $_POST['ten'];
        ...
        if ($d->insert($data)){
			...
			redirect("index.php?com=search&id_list=".$id_list."&act=man_cat");
		}
        else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man_cat");
    }
}
...
function delete_cat() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man_cat");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=search&id_list=".$id_list."&act=man_cat");
}
...
function get_photos() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "";
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_photo() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_photo() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
        redirect("index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    } { // them moi
        for ($i = 0; $i < 5; $i++) {
            if ($photo = upload_image("file" . $i, 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_search, $file_name . $i)) {
                ...
                $d->setTable('search_hinhanh');
                if (!$d->insert($data))
                    transfer("Lưu dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
            }
        }
    }
    redirect("index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
...
function delete_photo() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=search&id_list=".$id_list."&act=man_photo&idc=" . $_REQUEST['idc'] . "");
}
