...
switch ($act) {
    case "man":
        get_items();
        $template = "huong/items";
        break;
    case "add":
        $template = "huong/item_add";
        break;
    case "edit":
        get_item();
        $template = "huong/item_add";
...
function get_items() {
	...
	if(!empty($_POST)){
		...
		if($multi=='show'){
			for($i=0;$i<$count;$i++){
				$sql = "UPDATE table_huong SET hienthi =1 WHERE  id = ".$id_array[$i]."";
				mysql_query($sql) or die("Not query sqlUPDATE_ORDER");				
			}
			redirect("index.php?com=huong&act=man&type=".$type);			
		}
		
		if($multi=='hide'){
			for($i=0;$i<$count;$i++){
				$sql = "UPDATE table_huong SET hienthi =0 WHERE  id = ".$id_array[$i]."";
				mysql_query($sql) or die("Not query sqlUPDATE_ORDER");				
			}
			redirect("index.php?com=huong&act=man&type=".$type);			
		}
		...
		if($multi=='del'){
			for($i=0;$i<$count;$i++){
				...
				$sql = "select id from #_huong where id= ".$id_array[$i]."";
				$d->query($sql);
				if($d->num_rows()>0){
				...
				$sql = "delete from table_huong where id = ".$id_array[$i]."";
    ...
    $url = "index.php?com=huong&act=man";
    $maxR = 20;
    $maxP = 4;
...
function get_item() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=huong&act=man");
    $item = $d->fetch_array();
}
