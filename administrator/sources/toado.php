...
function get_items() {
    ...
    $url = "index.php?com=toado&idc=".$idc."&act=man";
    $maxR = 20;
    $maxP = 4;
...
function get_item() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=toado&idc=".$idc."&act=man");

    $sql = "select * from #_toado where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=toado&idc=".$idc."&act=man");
    $item = $d->fetch_array();
}
...
function save_item() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=toado&idc=".$idc."&act=man");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=toado&idc=".$idc."&act=man");
    }else {
		$data['idc'] = $idc;
        $data['ten_vi'] = $_POST['ten_vi'];
        $data['ten_en'] = $_POST['ten_en'];
        ...
        if ($d->insert($data))
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=toado&idc=".$idc."&act=man");
    }
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=toado&idc=".$idc."&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=toado&idc=".$idc."&act=man");
}
