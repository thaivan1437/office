...
function get_item() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=khuyenmai&act=man");
    $item = $d->fetch_array();
}
...
function save_item() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=khuyenmai&act=edit&id=".$id);
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=khuyenmai&act=edit&id=".$id);
    }else {
...
function get_cat() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=khuyenmai&act=man_cat");
    $item = $d->fetch_array();
}
