...
function get_items() {
    ...
    $url = "index.php?com=comment&act=man" . $urldanhmuc;
    $maxR = 20;
    $maxP = 20;
...
function get_item() {
    ...
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=comment&act=man");
    $item = $d->fetch_array();
}
...
function save_item() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=comment&act=man&curPage=" . $_REQUEST['curPage'] . "");
        else
...
function get_cats() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=comment&act=man_cat";
    $maxR = 20;
    $maxP = 10;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_cat() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_cat");

    $sql = "select * from #_comment_cat where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=comment&act=man_cat");
    $item = $d->fetch_array();
}
...
function save_cat() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=comment&act=man_cat&curPage=" . $_REQUEST['curPage'] . "");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=comment&act=man_cat");
    }else {
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_comment, $file_name)) {
            $data['photo'] = $photo;
            $data['thumb'] = create_thumb($data['photo'], 310, 310, _upload_comment, $file_name, 2);
        ...
        $d->setTable('comment_cat');
        if ($d->insert($data))
            redirect("index.php?com=comment&act=man_cat");
        else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=comment&act=man_cat");
    }
}
...
function delete_cat() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=comment&act=man_cat");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_cat");
}
...
function get_loais() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=comment&act=man_item";
    $maxR = 20;
    $maxP = 10;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_loai() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_item");

    $sql = "select * from #_comment_item where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=comment&act=man_item");
    $item = $d->fetch_array();
}
...
function save_loai() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=comment&act=man_item&curPage=" . $_REQUEST['curPage'] . "");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=comment&act=man_item");
    }else {
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_comment, $file_name)) {
            $data['photo'] = $photo;
            $data['thumb'] = create_thumb($data['photo'], 310, 310, _upload_comment, $file_name, 2);
        ...
        $d->setTable('comment_item');
        if ($d->insert($data))
            redirect("index.php?com=comment&act=man_item");
        else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=comment&act=man_item");
    }
}
...
function delete_loai() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=comment&act=man_item");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_item");
}
...
function get_lists() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=comment&act=man_list";
    $maxR = 20;
    $maxP = 10;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_list() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_list");
    $sql = "select * from #_comment_list where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=comment&act=man_list");
    $item = $d->fetch_array();
}
...
function save_list() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=comment&act=man_list&curPage=" . $_REQUEST['curPage'] . "");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=comment&act=man_list");
    }else {
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_comment, $file_name)) {
            $data['photo'] = $photo;
        }
        ...
        $d->setTable('comment_list');
        if ($d->insert($data))
            redirect("index.php?com=comment&act=man_list");
        else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=comment&act=man_list");
    }
}
...
function delete_list() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=comment&act=man_list");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_list");
}
...
function get_danhmucs() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=comment&act=man_danhmuc";
    $maxR = 20;
    $maxP = 10;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_danhmuc() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_danhmuc");
    $sql = "select * from #_comment_danhmuc where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=comment&act=man_danhmuc");
    $item = $d->fetch_array();
}
...
function save_danhmuc() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=comment&act=man_danhmuc&curPage=" . $_REQUEST['curPage'] . "");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=comment&act=man_danhmuc");
    }else {
        if ($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_comment, $file_name)) {
            $data['photo'] = $photo;
        ...
        $d->setTable('comment_danhmuc');
        if ($d->insert($data))
            redirect("index.php?com=comment&act=man_danhmuc");
        else
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=comment&act=man_danhmuc");
    }
}
...
function delete_danhmuc() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=comment&act=man_danhmuc");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_danhmuc");
}
...
function get_photos() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "";
    $maxR = 10;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function get_photo() {
    ...
    $id = isset($_GET['id']) ? themdau($_GET['id']) : "";
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    ...
    $d->select();
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    $item = $d->fetch_array();
    $d->reset();
}
...
function save_photo() {
    ...
    if ($id) { // cap nhat
        ...
        $d->setWhere('id', $id);
        if (!$d->update($data))
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
        redirect("index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    } { // them moi
        for ($i = 0; $i < 5; $i++) {
            if ($photo = upload_image("file" . $i, 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_comment, $file_name . $i)) {
                ...
                $d->setTable('comment_hinhanh');
                if (!$d->insert($data))
                    transfer("Lưu dữ liệu bị lỗi", "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
            }
        }
        redirect("index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    }
...
function delete_photo() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->delete())
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=man_photo&idc=" . $_REQUEST['idc'] . "");
}
...
function get_duyetbl() {
    ...
    $curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
    $url = "index.php?com=comment&act=duyetbl";
    $maxR = 20;
    $maxP = 4;
    $paging = paging($items, $url, $curPage, $maxR, $maxP);
    $items = $paging['source'];
...
function delete_binhluan() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            redirect("index.php?com=comment&act=duyetbl");
        else
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=comment&act=duyetbl");
    }elseif (isset($_GET['listid']) == true) {
        $listid = explode(",", $_GET['listid']);
        for ($i = 0; $i < count($listid); $i++) {
            $idTin = $listid[$i];
        ...
        transfer("Không nhận được dữ liệu", "index.php?com=comment&act=duyetbl");
}
