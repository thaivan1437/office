...
<script language="javascript">
    function select_onchange()
	{
		...
		window.location ="index.php?com=product&act=man_list&subcat=<?=$_REQUEST["subcat"]?>&type=<?=$_REQUEST["type"]?>&id_parents="+a.value;	
		return true;
	}
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=product&act=man_list&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>"><span>Danh mục</span></a></li>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=product&act=add_list&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>'" />
        <input type="button" class="blueB" value="Hiện" onclick="ChangeAction('index.php?com=product&act=man_list&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>&multi=show');return false;" />
        <input type="button" class="blueB" value="Ẩn" onclick="ChangeAction('index.php?com=product&act=man_list&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>&multi=hide');return false;" />
        <input type="button" class="blueB" value="Xoá" onclick="ChangeAction('index.php?com=product&act=man_list&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>&multi=del');return false;" />
    </div>
		...
		<tbody>
			...
			<tr>
				...
				<td class="title_name_data">
					<a href="index.php?com=product&act=edit_list&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>&id_list=<?= $items[$i]['id_parent'] ?>&id_parent=<?=$items[$i]["set_level"]?>" class="tipS SC_bold">
						<?=$items[$i]["ten_vi"]?>
					</a>
				</td>
				...
				<td class="actBtns">
					<a href="index.php?com=product&act=edit_list&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>&id_list=<?= $items[$i]['id_parent'] ?>&id_parent=<?=$items[$i]["set_level"]?>" title="" class="smallButton tipS" original-title="Sửa sản phẩm"><img src="./images/icons/dark/pencil.png" alt=""></a>
					<a href="" onclick="CheckDelete('index.php?com=product&act=delete_list&id=<?=$items[$i]['id']?>&subcat=<?=$_REQUEST["subcat"]?>&type=<?=$_REQUEST["type"]?>'); return false;" title="" class="smallButton tipS" original-title="Xóa sản phẩm"><img src="./images/icons/dark/close.png" alt=""></a>
				</td>
			</tr>
			<?php } ?>
		</tbody>
