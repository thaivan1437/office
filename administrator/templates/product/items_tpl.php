...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=product&act=man"><span>dự án</span></a></li>
...
<div class="widget">
  <div class="titlee" style="padding-bottom:5px;">
    ...
    <div class="timkiem" >
		<form name="search" action="" method="GET" class="form giohang_ser">
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=product&act=add&type=<?=$_REQUEST["type"]?>'" />
	...
	<div style="float:right;">
        <div class="selector">
			<select name="dist" id="dist" onchange="select_onchange1()">
				<option value="0">Danh mục quận</option>
				<?php foreach($dist as $v){?>
                <option value="<?=$v["id"]?>"><?=$v["ten"]?></option>
                <?php }?>
            </select>
        </div>  
    </div>  
    	<img src="./images/question-button.png" alt="Chọn loại" class="icon_que tipS" style="float:right; margin:5px 5px 0 0;" original-title="Dùng cây thư mục để di chuyển nhanh đến dự án">   
</div>
...
<div class="widget">
	<div class="title">
		...
		<h6>Danh sách các dự án hiện có</h6>
	</div>
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
		<thead>
			<tr>
				...
				<td class="sortCol"><div>Tên dự án<span></span></div></td>
				<td class="tb_data_small">Ẩn/Hiện</td>
				<td width="100">Nổi bật</td>
				<td width="100">Giá rẻ</td>
		...
		<tbody>
			...
			<tr>
				...
				<td class="title_name_data">
					<a href="index.php?com=product&act=edit&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST["type"]?>&id_list=<?=$items[$i]['id_list']?>&id_parent=<?=$items[$i]['list_id']?>" class="tipS SC_bold">
						<?=$items[$i]["ten_vi"]?>
					</a>
				</td>
				<td align="center">
				...
				<td class="actBtns">
					<a href="index.php?com=product&act=edit&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST["type"]?>&id_list=<?=$items[$i]['id_list']?>&id_parent=<?=$items[$i]['list_id']?>" title="" class="smallButton tipS" original-title="Sửa dự án"><img src="./images/icons/dark/pencil.png" alt=""></a>
					<a href="" onclick="CheckDelete('index.php?com=product&act=delete&id=<?=$items[$i]['id']?>&type=<?=$_REQUEST["type"]?>'); return false;" title="" class="smallButton tipS" original-title="Xóa dự án"><img src="./images/icons/dark/close.png" alt=""></a>
				</td>
			</tr>
			<?php } ?>
		</tbody>
...
<script type="text/javascript">
	...
	function select_onchange()
	{
		...
		window.location ="index.php?com=product&act=man&type=<?=$_REQUEST["type"]?>&id_list="+a.value;	
		return true;
	}
	function select_onchange1()
	{
		var b=document.getElementById("dist");
		window.location ="index.php?com=product&act=man&type=<?=$_REQUEST["type"]?>&dist="+b.value;	
		return true;
	}
</script>
<?php 
