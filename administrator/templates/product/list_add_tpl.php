...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        ...
        <li><a href="index.php?com=product&act=man&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>"><span>Danh mục sản phẩm</span></a></li>
        <li class="current"><a href="#" onclick="return false;">Thêm</a></li>
        </ul>
        <div class="clear"></div>
    </div>
</div>
<form name="supplier" id="validate" class="form" action="index.php?com=product&act=save_list&type=<?=$_REQUEST["type"]?>&subcat=<?=$_REQUEST["subcat"]?>&curPage=<?=$_REQUEST['curPage']?>" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<div id="info" class="tab_content">
			...
			<div class="formRow">
				...
				<div class="formRight">
					<div class="selector">
						<select name="value_menu">
							<option value="1" <?=($item["value_menu"]==1)?"selected":"";?>> Hiển thị danh mục cấp 2</option>
							<option value="2" <?=($item["value_menu"]==2)?"selected":"";?>> Hiển thị quận</option>
							<option value="3" <?=($item["value_menu"]==3)?"selected":"";?>> Hiển thị cả 2</option>
						</select>
					</div>
				</div>
				<div class="clear"></div>
			...
			<?php }?>
			<?php if($_REQUEST["type"]=="san-pham" || $_REQUEST["type"]=="van-phong-tron-gai" || $_REQUEST["type"]=="toa-nha-nguyen-can"){?>
			<div class="formRow">
				<label>Font icon: </label>
				<div class="formRight">
					<input type="text" id="fontawesome" name="fontawesome" value="<?=@$item['fontawesome']?>"  title="Nhập font awesome" class="tipS" />
				</div>
				<div class="clear"></div>
			</div>
			<div class="formRow">
				<label>Loại vp: </label>
				<div class="formRight">
					<input type="text" id="fontawesome" name="typeloai" value="<?=@$item['typeloai']?>"  title="Type Loại" class="tipS" />
				</div>
				<div class="clear"></div>
			</div>
			<div class="formRow">
				...
				<div class="formRight">	
					...
					<label><input type="checkbox" name="dist[]" value="<?=@$v['id']?>" <?=(check_dist($v["id"],$item["dist"])==1) ? "checked" : "";?> /> <?=$v["ten"]?></label>
					<?php }?>
				</div>
				<div class="clear"></div>
			</div>
